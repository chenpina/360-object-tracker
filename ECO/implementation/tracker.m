...
for frame = 1:num_frames
    ...
    if params.use360
        ori_im = im;
        center = pos;
        f_scale = max(target_sz/re_scale)*params.search_area_scale*1.2;
...
%     fprintf(fid, '\tTarget localization step: %f s(%f)\n', toc(), toc()-tmp_time);
%     fprintf(fid, '\tModel update step:\n');
%     tmp_time = toc();
...
%     fprintf(fid, '\tExtract sample: %f s(%f)\n', toc(), toc()-tmp_time);
%     tmp_time = toc();
...
%     fprintf(fid, '\tMerge sample & weight: %f s(%f)\n', toc(), toc()-tmp_time);
%     fprintf(fid, '\tTrain tracker:\n');
%     tmp_time = toc();
...
%         set(gcf,'units','normalized','outerposition',[0 0 1 1]);
    ...
    % visualization
    if visualization == 1
        rect_position_vis = [result_pos([2,1]) - (target_sz([2,1]) - 1)/2, target_sz([2,1])];
        ...
        if frame == 1  %first frame, create GUI
            ...
            imshow(im_to_show);axis off;axis image;
